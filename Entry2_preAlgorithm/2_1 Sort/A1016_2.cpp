...
int main()
{
    ...
    {
        next1 = start;
        while (strcmp(bill[next1].name, bill[start].name) == 0)
        {
            bill[next1].id = order;
            next1++;
        }
...
/*
    ...
    */
    ...
    for (int i = 0; i < order; i++)
    {
        ...
        next = on + 1;
        while (next < n && bill[on].id == i && bill[next].id == i)
        {
            int time = 0, price = 0;
            if (bill[on].state == true && bill[next].state == false) //有配套on-off
            {
                print(on, next, time, price); //输出函数

                allPrice += price;
                printf("%d $%.2f\n", time, price / 100.0);

                on = next + 1; //处理完一组on-off, 指向下一个
                next = on + 1;
            }
            else
            {
